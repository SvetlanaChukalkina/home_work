...
def test_filter_by_state(state: str, expected: list[dict]) -> None:
...
@pytest.fixture
def sample_data() -> list[dict[Any, Any]]:
...
def test_filter_by_no_state(sample_data: list[dict[Any, Any]]) -> None:
    result = filter_by_state(sample_data, state="DELETED")
    assert result == []
...
def test_sort_by_date(date: list[dict], expected: list[dict]) -> None:
...
@pytest.fixture
def not_correct_date() -> list[dict[str, str]]:
    """Проверка работы функции с некорректными форматами дат"""
    return [{"date": "12-03-18"}]


def test_not_correct_date(not_correct_date: list[dict]) -> None:
    """Проверка работы функции с некорректными форматами дат"""
